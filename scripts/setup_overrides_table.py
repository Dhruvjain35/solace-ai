"""Create the Solace AI override / provenance log table. Idempotent.

`solace-ai-overrides` is the durable backing store for backend/lib/provenance.py —
it persists each AI override-decision (accepted / edited / rejected) so the public
HTI-1 DSI transparency metrics survive Lambda cold starts.
...
Schema:
  ...
  - TTL attribute: ttl               — 90-day hot retention in DDB; the 6-year HIPAA /
                                       HTI-1 record lives in S3 (CMK JSONL) via storage.py
...
DO NOT run against AWS as part of CI — invoke manually when provisioning a workspace:
    python scripts/setup_overrides_table.py
"""
...
REGION = "us-east-1"
TABLE_NAME = "solace-ai-overrides"
TTL_ATTRIBUTE = "ttl"
...
def main() -> None:
    ...
    setup_table(cmk_arn)
    print(
        "\nDone. solace-ai-overrides is the durable store for lib.provenance "
        "(HTI-1 override metrics)."
    )
